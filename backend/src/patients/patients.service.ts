...
export class PatientService {
  ...
  async getPatients(clinicId: string): Promise<any> {
    try {
      ...
      const [response, listInsurancePatients, listVisitsPatients] =
        await Promise.all([
      ...
      const insurancesMap: Record<
        string,
        { contactId: string; carrier_title: string | null }
      > = Object.fromEntries(
        listInsurancePatients.map((insurance) => [
          insurance.contactId,
          insurance,
        ]),
      )
  ...
  async getPaginatedPatients(clinicId, page, pageSize) {
    try {
      ...
      const patients = contacts.filter((contact) => contact.type === 'PATIENT')
