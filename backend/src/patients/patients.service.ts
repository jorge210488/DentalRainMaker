...
export class PatientService {
  ...
  async getPatients(clinicId: string): Promise<any> {
    ...
    try {
      ...
      const [response, listInsurancePatients, listVisitsPatients] = await Promise.all([
        lastValueFrom(this.httpService.get(url, { headers })),
        this.insuranceService.getAllInsuranceCoverage(clinicId),
        this.appointmentsService.getVisits(clinicId)
      ]) 
      ...
      const insurancesMap: Record<string, {contactId: string; carrier_title: string | null}> = Object.fromEntries(
        listInsurancePatients.map((insurance) => [insurance.contactId, insurance])
      );
      ...
      const patients = contacts
        ...
        .map((patient) => {
          ...
          const insuranceData = insurancesMap[patient.remote_id];
          ...
          return {
            ...
            insurance: insuranceData?.carrier_title || null,
